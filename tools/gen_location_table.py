...
# Same enumeration order as gen_item_table.py, but producing LOCATION ids (one per
# real in-game "purchase this upgrade/equip this gear" action) keyed by the exact same
# "slotId:upgradeName" / "gearType:equipmentName" string used as the matching item's
# payload, so BomberCrew/ShopHooks.cs can look a purchase straight up by that key.
...
for slot_id, slot_type, default_name in SLOTS:
    for item_name, item_type in BOMBER_CATALOGUE:
        ...
        if item_name == default_name:
            continue
        bomber_keys.append(f"{slot_id}:{item_name}")

crew_keys = []  # "gearType:equipmentName"
for item_name, gear_type in CREW_CATALOGUE:
    if item_name.endswith("None"):
        continue
    crew_keys.append(f"{gear_type}:{item_name}")

print(f"-- bomber purchase locations: {len(bomber_keys)}")
print(f"-- crew purchase locations: {len(crew_keys)}")

BOMBER_LOCATION_BASE = 9210000
CREW_LOCATION_BASE = 9220000

lines = []
for i, key in enumerate(bomber_keys):
    loc_id = BOMBER_LOCATION_BASE + i
    esc_key = key.replace('"', '\\"')
    lines.append(f'        {{ "{esc_key}", {loc_id} }},')

lines.append("")  # visual separator between the two blocks in the generated output

for i, key in enumerate(crew_keys):
    loc_id = CREW_LOCATION_BASE + i
    esc_key = key.replace('"', '\\"')
    lines.append(f'        {{ "{esc_key}", {loc_id} }},')

with open("gen_location_output.txt", "w") as f:
    f.write("\n".join(lines))

print("wrote", len(lines), "lines to gen_location_output.txt")
